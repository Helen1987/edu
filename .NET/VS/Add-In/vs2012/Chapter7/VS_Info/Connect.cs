		...
		public void OnConnection(object application, ext_ConnectMode connectMode, object addInInst, ref Array custom)
		{
			...
			if(connectMode == ext_ConnectMode.ext_cm_UISetup)
			{
				...
				try
				{
					...
					Command command = commands.AddNamedCommand2(_addInInstance, "VS_Info", "VS_Info", "Executes the command for VS_Info", true, 59, ref contextGUIDS, (int)vsCommandStatus.vsCommandStatusSupported+(int)vsCommandStatus.vsCommandStatusEnabled, (int)vsCommandStyle.vsCommandStylePictAndText, vsCommandControlType.vsCommandControlTypeButton);
		...
		public void Exec(string commandName, vsCommandExecOption executeOption, ref object varIn, ref object varOut, ref bool handled)
		{
			...
			if(executeOption == vsCommandExecOption.vsCommandExecOptionDoDefault)
			{
				if(commandName == "VS_Info.Connect.VS_Info")
				{
                    ...
                    sb.AppendLine("            User: " + Environment.UserDomainName + "/" + Environment.UserName.ToString());
                    sb.AppendLine("Operating System: " + OSVersionToFriendlyName(Environment.OSVersion.Version.Major, Environment.OSVersion.Version.Minor));
                    sb.AppendLine("                  " + Environment.OSVersion.ToString() + " with " + Environment.ProcessorCount.ToString() + " processors");

                    if (Environment.OSVersion.Platform == PlatformID.Win32Windows || Environment.OSVersion.Platform == PlatformID.Win32Windows)
                    {
